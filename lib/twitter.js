...
var Auth = require('./auth')
  ...
  , TwitterMedia = require('twitter-media')
...
Twitter.prototype = {
  ...
  uploadMedia: function (params, callback) {
    ...
    twitterMedia.uploadMedia(params.type, params.media, callback)
  },
  request: function (method, path, params, callback) {
    ...
    var allowDirtyMethods = params.allow_dirty_methods || false;
    if (params.allow_dirty_methods) {
      delete params.allow_dirty_methods;
    }
    ...
    if (path === 'media/upload') {
      if (!allowDirtyMethods) {
        console.warn('Twit: Uploading media should be done by method uploadMedia');
        return callback();
      }
      // build the full url
